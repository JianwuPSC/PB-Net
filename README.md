...
The model was trained on a dataset of 14,331 items and tested on a set of 1,480 items. The dataset was partitioned in a manner consistent with the standard practice for PPI prediction to prevent data leakage. The SMOTE was applied to augment the minority class samples
    ...
    python model/PB-Net_item_train.py \
  --train_source_esm embedding/train_esm_represent_source_df.csv \
  --train_binder_esm embedding/train_esm_represent_binder_df.csv \
  --train_target_esm embedding/train_esm_represent_target_df.csv \
  --train_source_saprot embedding/train_saprot_represent_source_df.csv \
  --train_binder_saprot embedding/train_saprot_represent_binder_df.csv \
  --train_target_saprot embedding/train_saprot_represent_target_df.csv \
  --valid_source_esm embedding/valid_esm_represent_source_df.csv \
  --valid_binder_esm embedding/valid_esm_represent_binder_df.csv \
  --valid_target_esm embedding/valid_esm_represent_target_df.csv \
  --valid_source_saprot embedding/valid_saprot_represent_source_df.csv \
  --valid_binder_saprot embedding/valid_saprot_represent_binder_df.csv \
  --valid_target_saprot embedding/valid_saprot_represent_target_df.csv \
  --test_source_esm embedding/test_esm_represent_source_df_all.csv \
  --test_binder_esm embedding/test_esm_represent_binder_df_all.csv \
  --test_target_esm embedding/test_esm_represent_target_df_all.csv \
  --test_source_saprot embedding/test_saprot_represent_source_df_all.csv \
  --test_binder_saprot embedding/test_saprot_represent_binder_df_all.csv \
  --test_target_saprot embedding/test_saprot_represent_target_df_all.csv \
  --mode cat_saprot_source_binder_target_esm2_match_source_binder_target \
  --save_model model/IFM/IFM.pt \
  --save_predictions model/IFM/IFM_train \
  --balance_strategy smote \
  --pos_weight 2.0 \
  --batch_size 32 --epochs 100 --patience 20 --monitor f1 --monitor_mode max --seed 12
### (3) PPI prediction
