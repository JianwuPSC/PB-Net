...
### (2) IFM training
The model was trained on a dataset of 14,878 items and tested on a set of 3,614 items. The dataset was partitioned in a manner consistent with the standard practice for PPI prediction to prevent data leakage. The SMOTE was applied to augment the minority class samples
...
### (3) PPI prediction
In this IFM, the model performs binary classification where class 0 represents a true interaction (PPI positive) and class 1 indicates a true interaction (PPI positive). The classification threshold is set at 0.5: predicted probability scores greater than 0.5 are assigned to class 1 (interacting pairs), while scores below 0.5 are classified as non-interacting pairs (class 0). The model outputs a continuous probability score ranging from 0 to 1, where higher values correspond to increased confidence in the predicted interaction. 
