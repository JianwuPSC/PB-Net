...
for path in paths:
    ...
    if str(path).find(str(key_word)) != -1:
        ...
        for b in range(len(A_chain)):
            ...
            min_contact  = np.min(np.array(dis1))
            dis_dict[name] = dis1
            count_dict[name] = count
            count_dict1[name] = count1
            near_dict[name] = min_contact

####################
f=open(file_out,"w")
for keys in count_dict.keys():
    f.write(keys+'\t'+str(count_dict1[keys])+'\t'+str(count_dict[keys])+'\t'+str(near_dict[keys])+'\n')
f.close()
